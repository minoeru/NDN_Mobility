...
makeResult <- function(x){
  for (i in 0:file_num[number]) {
    ...
    if(i == 0) result <- data.frame(name="nlsr_pc",all=nrow(ndn),interest,data,info_interest,info_reply,LSA_interest,LSA_data,sync_interest,sync_data,arp,other,sync,sync_len,sync_interest_len,sync_data_len)
    else result <- data.frame(name=paste0("nlsr",i),all=nrow(ndn),interest,data,info_interest,info_reply,LSA_interest,LSA_data,sync_interest,sync_data,arp,other,sync,sync_len,sync_interest_len,sync_data_len) %>% rbind(result,.)
  }
...
makeCSV <- function(my_arg){
  ...
  name <- c(df[[1]]$name,"ave","r_sum","r_ave","r_sum1-6","r_ave1-6","r_sum7-10","r_ave7-10")
  tmp <- nrow(df[[1]]) -1
  tmp_data <- sapply(1:length(data_num), function(x){
    c(df[[x]][,my_arg],mean(df[[x]][,my_arg][1:tmp]),sum(df[[x]][,my_arg][2:tmp]),mean(df[[x]][,my_arg][2:tmp]),
      sum(df[[x]][,my_arg][2:7]),mean(df[[x]][,my_arg][2:7]),sum(df[[x]][,my_arg][8:11]),mean(df[[x]][,my_arg][8:11]))
  })
  tmp_result <- data.frame(name,tmp_data)
  names(tmp_result) <- c("name",paste0(my_arg,data_num))
  file_name <- ifelse(SELF_FLAG,"_detail_self.csv","_detail.csv") 
  write.csv(tmp_result,file = paste0(dir[number],"/result/statis_detail/nlsr_",my_arg,file_name),row.names = F)
  
  file_name <- ifelse(SELF_FLAG,"_inc_self.csv","_inc.csv") 
  result_tmp_increase <- sapply(1:length(data_num), function(x){ tmp_result[x+1] / tmp_result[2] }) %>% data.frame(name,.)
  write.csv(result_tmp_increase,file = paste0(dir[number],"/result/statis_increase/nlsr_",my_arg,file_name),row.names = F)
}

file_name <- ifelse(SELF_FLAG,"/nlsr_self.csv","/nlsr.csv") 
for (i in data_num) write.csv(makeResult(i),file = paste0(dir[number],"/result/ndn_csv/",i,file_name),row.names = F)
# for (i in c("LSA_interest","LSA_data","sync","sync_interest","sync_data")) makeCSV(i)